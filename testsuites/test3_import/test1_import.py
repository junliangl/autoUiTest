...
class Test_Import_Page(unittest.TestCase):
    ...
    def test1_check_import_setting_exist(self):
        """
        测试查看是否有入库设置
        """
        ...
        if result:
            self.assertTrue(result, logger.info('存在入库设置.'))
        else:
            self.assertTrue(result, logger.error('不存在入库设置!'))

    def test2_enter_import_page(self):
        """
        测试进入导入页
        """
        import_page = Import_File_Page(self.driver)
        ...
        if result:
            self.assertTrue(result, logger.info('进入导入页没有问题.'))
        else:
            self.assertTrue(result, logger.error('进入导入页存在问题!'))

    def test3_check_buttons(self):
        """
        测试查看默认buttons
        """
        import_page = Import_File_Page(self.driver)
        ...
        if result:
            self.assertTrue(result, logger.info('查看buttons没有问题.'))
        else:
            self.assertTrue(result, logger.error('buttons存在问题!'))

    def test4_switch_buttons(self):
        """
        测试切换按钮
        """
        import_page = Import_File_Page(self.driver)
        ...
        if result:
            self.assertTrue(result, logger.info('切换按钮没有问题.'))
        else:
